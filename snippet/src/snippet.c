/* ========================================================================== */
/* File: snippet.c - Snippet accomplishment tracker
 *
 ...
 * Usage:
 *
 * 	`snippet -p "My project" -a "My accomplishment"`
 * 	  Append accomplishment specific to a specific project; if no project is
 * 	  specified, "Miscellaneous" is used by default
 *
 * 	`snippet -e <entry_id>`
 * 	  Edit entry, either changing the project or accomplishment
 *
 * 	`snippet --list-projects`
 * 	  List projects that have been documented
 *
 * 	`snippet -p "My project"`
 * 	  List historic accomplishments for a specific project
 * 
 * TODO:
 ...
 * 	[x] List projects:
 *
 * 	[x] List historic accomplishments:
 *
...
#include "utils.h"                           // utility functions and definitions
#include "hashtable.h"			     // hash table functionality

// ---------------- Constant definitions
#define OUTPUT_FILE "sample.txt"
...
int projectAccomplishments(char* line);
void projectList();
...
int main(int argc, char **argv) {
	...
	if ((entry == 0) && (group == 1)) {
	  	//fprintf(stderr, "Error receiving project with no accomplishment\n"
		//        "snippet -p 'My project' -a 'My accomplishment'\n");
		readFile(project, projectAccomplishments);
		SAFEFREE(project);
		//return EXIT_FAILURE;
	}
	else if (entry == 1) {
